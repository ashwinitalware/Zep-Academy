...
export class RefundpolicyPage implements OnInit {
  ...
  get_refund_policy() {
    ...
    this.http.post(`${this.url.serverUrl}get_refund_policy`, requestData).subscribe(
      (res: any) => {
        if (res.status && res.data && res.data.refund) {
          this.allrefunddata.push(res.data);
          console.log(res, 88);
        } else {
