...
export class ShippingpolicyPage implements OnInit {
  ...
  get_shipping_policy() {
    ...
    this.http.post(`${this.url.serverUrl}get_shipping_policy`, requestData).subscribe(
      (res: any) => {
        if (res.status && res.data && res.data.shipping) {
          this.allshippingdata.push(res.data);
          console.log(res, 88);
        } else {
